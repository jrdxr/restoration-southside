...
interface Env {
	...
	WEBFLOW_CATEGORIES_COLLECTION_ID: string;
}
	...
	try {
		...
		for (const [pcId, wfEvent] of Object.entries(webflowEvents)) {
			if (!pcEventIds.has(pcId)) {
				try {
					const eventName = (wfEvent as any).fieldData?.name || pcId;
					console.log(`Deleting: ${eventName}`);
					await deleteWebflowEvent((wfEvent as any).id, env);
					result.deleted++;
					result.deletedItems.push(eventName);
				} catch (error: any) {
					console.error(`Error deleting ${pcId}:`, error.message);
					result.errors.push({
						event: (wfEvent as any).fieldData?.name || pcId,
						error: error.message
					});
				}
			}
...
async function fetchPlanningCenterEvents(env: Env) {
	...
	for (const signup of data.data || []) {
		const catIds = signup.relationships?.categories?.data?.map((c: any) => c.id) || [];
		const categories = included
			.filter((i: any) => i.type === 'Category' && catIds.includes(i.id))
			.map((c: any) => c.attributes.name);

		const timeIds = signup.relationships?.signup_times?.data?.map((t: any) => t.id) || [];
		const times = included.filter((i: any) => i.type === 'SignupTime' && timeIds.includes(i.id));
...
async function webflowFetch(path: string, env: Env, options: any = {}) {
...
async function getWebflowCategories(env: Env) {
	const data: any = await webflowFetch(
...
async function getWebflowEvents(env: Env) {
	const data: any = await webflowFetch(
...
async function createWebflowEvent(event: any, categoryMap: any, env: Env) {
	const categoryIds = event.categories
		.map((name: string) => categoryMap[name])
		.filter(Boolean);
...
async function updateWebflowEvent(webflowId: string, event: any, categoryMap: any, env: Env) {
	const categoryIds = event.categories
		.map((name: string) => categoryMap[name])
		.filter(Boolean);
...
async function deleteWebflowEvent(webflowId: string, env: Env) {
	const collectionPath = `/collections/${env.WEBFLOW_EVENTS_COLLECTION_ID}/items/${webflowId}`;

	const deleteRequest = (path: string) => fetch(`${WF_BASE_URL}${path}`, {
		method: 'DELETE',
		headers: {
			'Authorization': `Bearer ${env.WEBFLOW_API_TOKEN}`,
			'accept': 'application/json'
...
function hasChanged(existingEvent: any, newEvent: any, categoryMap: any, lastState: any) {
	...
	const newCats = newEvent.categories.map((name: string) => categoryMap[name]).filter(Boolean);
	const catsChanged = existingCats.length !== newCats.length ||
		!existingCats.every((id: string) => newCats.includes(id)) ||
		!newCats.every((id: string) => existingCats.includes(id));
...
async function saveHistory(env: Env, result: any) {
        ...
        ${sync.createdItems && sync.createdItems.length > 0 ? `
          <div class="details-section">
            ...
            <table>
              ...
              <tbody>
                ${sync.createdItems.map((name: string) => `<tr><td>${name}</td></tr>`).join('')}
              </tbody>
            </table>
          </div>
        ` : ''}
        ...
        ${sync.updatedItems && sync.updatedItems.length > 0 ? `
          <div class="details-section">
            ...
            <table>
              ...
              <tbody>
                ${sync.updatedItems.map((item: any) => `
                  <tr>
                    <td>${item.name}</td>
                    <td>
                      ${item.changes.map((change: string) => `<div class="change-detail">• ${change}</div>`).join('')}
                    </td>
                  </tr>
                `).join('')}
              </tbody>
        ...
        ${sync.deletedItems && sync.deletedItems.length > 0 ? `
          <div class="details-section">
            ...
            <table>
              <thead><tr><th>Event Name</th></tr></thead>
              <tbody>
                ${sync.deletedItems.map((name: string) => `<tr><td>${name}</td></tr>`).join('')}
              </tbody>
            </table>
          </div>
        ` : ''}
